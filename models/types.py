TYPE_CHART_GEN2 = {
    ...
    "Ice":      {"Fire": 0.5, "Water": 0.5, "Grass": 2.0,
    ...
    "Rock":     {"Fire": 2.0, "Ice": 2.0, "Fighting": 0.5,
                 ...
                 "Steel": 0.5},
    "Ghost":    {"Normal": 0.0, "Psychic": 2.0,
                 "Ghost": 2.0, "Dark": 0.5},
    "Dragon":   {"Dragon": 2.0, "Steel": 0.5},
    "Dark":     {"Fighting": 0.5, "Psychic": 2.0,
                 "Ghost": 2.0, "Dark": 0.5, "Steel": 0.5},
...
def type_effectiveness(move_type: str, defender_types: list[str]) -> float:
    """Return the effectiveness multiplier for a move hitting a Pokémon.
       Normalizes type names (title-case, strips whitespace)."""
    # Normalize attacker type
    atk_type = move_type.strip().title()

    mult = 1.0
    for t in defender_types:
        def_type = t.strip().title()
        mult *= TYPE_CHART_GEN2.get(atk_type, {}).get(def_type, 1.0)
    return mult